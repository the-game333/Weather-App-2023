...
const TempApp = () => {
  ...
  useEffect(() => {
    const fetchApi = async () => {
      const url = process.env.link || `https://api.openweathermap.org/data/2.5/weather?q=${search}&units=metric&appid=ec95667ef545cb054ebd779e272fa5f4`;
  ...
  return (
    <>
      ...
      <div className="box p-3" style={{ backgroundColor: bg }}>
        ...
        {/* for displaying icon */}
        {weather.main && (
          <img src= {process.env.img || `https://openweathermap.org/img/wn/${weather.weather[0].icon}@2x.png`} className="iconss" alt="" />
        )}
